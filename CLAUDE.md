...
A Python MCP server built on `modelcontextprotocol/python-sdk` that exposes Gmail capabilities to Claude Code over stdio transport. Managed with `uv`.

## Directories

- `src/gmail_mcp/` — main package; see its own CLAUDE.md
- `docs/` — public documentation
- `local/` — gitignored; local dev notes and secrets, never committed

## Dev commands

```bash
uv sync          # install / sync dependencies
uv run gmail-mcp # run the server
